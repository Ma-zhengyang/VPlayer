...
public class BitmapManager {
        ...
        try {
            synchronized (status) {
                while (status.mThumbRequesting) {
                    MediaStore.Images.Thumbnails.cancelThumbnailRequest(cr, -1, t.getId());
                    MediaStore.Video.Thumbnails.cancelThumbnailRequest(cr, -1, t.getId());
                    status.wait(200);
                }
            }
    ...
    public Bitmap getThumbnail(ContentResolver cr, long origId, int kind,
        ...
        try {
            ...
            return MediaStore.Video.Thumbnails.getThumbnail(cr, origId, t.getId(), kind,
                    null);
        } finally {
            synchronized (status) {
