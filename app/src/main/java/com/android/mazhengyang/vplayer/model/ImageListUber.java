...
import static com.android.mazhengyang.vplayer.utils.Util.indexOf;
...
public class ImageListUber implements IImageList {
	...
	public ImageListUber(IImageList[] sublist, int sort) {
		...
		mQueue = new PriorityQueue<>(
				4,
				sort == ImageManager.SORT_BY_ASCENDING ? new AscendingComparator()
	...
	public synchronized int getImageIndex(IImage image) {
		IImageList list = image.getContainer();
		int listIndex = indexOf(mSubList, list);
		if (listIndex == -1) {
